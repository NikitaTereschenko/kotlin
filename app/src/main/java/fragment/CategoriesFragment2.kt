...
import com.example.shopapp.Adapters.PopularItemsAdapterAdmin
...
import com.example.shopapp.databinding.FragmentCategories3Binding
import com.example.shopapp.programData
import kotlin.collections.ArrayList

class CategoriesFragment2 : Fragment() {

    private lateinit var binding: FragmentCategories3Binding
    private lateinit var adapter : PopularItemsAdapterAdmin
    private lateinit var adapter2 : PopularItemsAdapter
    ...
    override fun onCreateView(
        ...
        binding = FragmentCategories3Binding.inflate(inflater, container, false)
        ...
        adapter = PopularItemsAdapterAdmin(requireContext(), list)
        adapter2 = PopularItemsAdapter(requireContext(), list)
        ...
        if (programData.adminFlag) {
            ...
            adapter2.setSharedModel(sharedModel)
            binding.categoriesRV2.adapter = adapter2
        }
        ...
        binding.addNew.setOnClickListener {
            ...
            var amount = 5000
            var image = "item1"
            name = programData.isExistItemName(name).toString()

            list.add(PopularModel(R.drawable.item1, name, 5000, "Тестовый товар", 1))
            adapter.updateList(list)
            programData.addObject(name, amount, image)
